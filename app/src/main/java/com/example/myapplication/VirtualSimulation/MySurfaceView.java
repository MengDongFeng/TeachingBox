package com.example.myapplication.VirtualSimulation;
...
//    private SceneRenderer mRenderer;  //这是个继承类，场景渲染器
    ...
    public MySurfaceView(Context context) {     //继承GLSurfaceView的构造器
        ...
        SceneRenderer mRenderer = new SceneRenderer();    //新建场景渲染器
    ...
    private class SceneRenderer implements Renderer {
        ...
        public void onSurfaceChanged(GL10 gl, int width, int height) {
            ...
            gl.glDepthFunc(GL10.GL_LEQUAL);//启用并设置深度检测的类型
            //设置视窗大小及位置
        ...
        public void onDrawFrame(GL10 gl) {
            ...
            */
            MatrixUtil.setEulerZYX(trans_1.basis, euler_1[2], euler_1[1], euler_1[0]);
            MatrixUtil.setEulerZYX(trans_2.basis, euler_2[2], euler_2[1], euler_2[0]);
            MatrixUtil.setEulerZYX(trans_3.basis, euler_3[2], euler_3[1], euler_3[0]);
            MatrixUtil.setEulerZYX(trans_4.basis, euler_4[2], euler_4[1], euler_4[0]);
            MatrixUtil.setEulerZYX(trans_5.basis, euler_5[2], euler_5[1], euler_5[0]);
            ...
            */
            dynamicsWorld.stepSimulation(1f /20f, 1); //模拟运动
            synchronized (tca) {   //同步
                //显示所有零件
    ...
    public void motion_calculate() {
        ...
        for (int i = 0; i < 7; i++)
        {
            //            int[] joint_temp={30.1f,40.2f,50.3f,-20.1f,-30.6,-40.3,-50.2};
            int[] joint_temp=new int[7];
            joint_temp[i] = (int) (joint_target[i]);
            if (joint_temp[i] != joint_current[i]) {
                if (joint_temp[i] - joint_current[i] > 1) {
                    joint_current[i] += 1;
                } else if (joint_temp[i] - joint_current[i] < -1) {
                    joint_current[i]-= 1;
                } else {
                    joint_current[i] = joint_temp[i];
                }
                // break;
            }
        }
...
*/
      /*  float joint_changed=0;
        float[] per_change=new float[7];
        {
            for(int i=0;i<7;i++){   //此处求关节角改变量的最大值
                float temp=abs(joint_target[i]-joint_current[i]);
                if (temp>joint_changed){
                    joint_changed=temp;
                }
            }

            for(int i=0;i<7;i++){
                per_change[i]= (abs(joint_target[i]-joint_current[i])/joint_changed);
            }
        }
        for (int i = 0; i < 7; i++)
        {
         //   joint_target[i] = (int) (joint_target[i]);
            if (joint_target[i] != joint_current[i]) {
                if (joint_target[i] - joint_current[i] > 0.1) {
                    joint_current[i] += per_change[i];
                } else if (joint_target[i] - joint_current[i] < -0.1) {
                    joint_current[i]-= per_change[i];
                } else {
                    joint_current[i] = joint_target[i];
                }
        ...
        cc2 = (float) (cos((joint_current[1]) * PI / 180));ss2= (float) (sin((joint_current[1]) * PI / 180));
