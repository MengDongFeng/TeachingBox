...
public class LoadUtil {
    ...
    public static LoadedObjectVertexNormal loadFromFile(int inde, String fname, Resources r, MySurfaceView mv){
        //加载后物体的引用
        LoadedObjectVertexNormal lo=null;
        //原始顶点信息列表--直接从OBJ文件中加载
        ...
        try {
            //桥接流，修饰流
            InputStream in=r.getAssets().open(fname);   //所有的输入流的基类 ，抽象基类
            InputStreamReader isr=new InputStreamReader(in);
            //转换流，要InputStream或OutputStream作为参数，实现字节流和字符流之间的转换
            ...
            while ((temps=br.readLine())!=null){
                ...
                if (tempsa[0].trim().equals("v")){            //trim():去掉字符串首尾的空格。
                    //此行 为顶点坐标,若为顶点坐标行则提取出此顶点的XYZ坐标添加到原始顶点坐标列表中
                    ...
                    //此行为三角形面
                    /*
                     *若为三角形面行,则根据组成面的顶点的索引从原始顶点坐标列表中
                     *提取相应的顶点坐标值添加到结果顶点坐标列表中，
                     * 同时根据三个顶点的坐标计算出此面的法向量
                     *并添加到平均前各个索引对应的点的法向量集合组成的Map中
                     */
                    ...
                    for (int tempIndex:index){
                        //记录每个索引点的法向量到平均各个对应的点的法向量集合组成索引的Map中
                        //获取当前索引对应点的法向量集合
            ...
            //创建3D物体对象
            lo=new LoadedObjectVertexNormal(inde,mv,vXYZ,nXYZ);
        }
        catch(Exception e){
